...
const fetchUser = async () => {
  try {
    const response = await _axios.get(`${import.meta.env.VITE_BACKEND_API_URL}/users/${userId}`,{
...
watch(matchingPassword, (newValue) => {
  if (newValue !== user.value.password) {
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <input v-model="user.first_name" id="first_name" type="text" class="form-control" required/>
          </div>
          <div class="col-md-6 mb-3">
            <label for="last_name" class="form-label">Last Name</label>
            <input v-model="user.last_name" id="last_name" type="text" class="form-control" required/>
          </div>
        </div>
        <div class="row">
          <div class="col-md-8 mb-3">
            <label for="job_title" class="form-label">Job Title</label>
            <input v-model="user.job_title" id="job_title" type="text" class="form-control" required/>
          </div>
        ...
        <div class="row">
          <div class="col-md-3 mb-3">
              ...
              <input :value="user.login_attempts" id="login_attempts" class="form-control" disabled></input>
          </div>
            <div :class="isEditMode ? 'col-md-9 mb-4' : 'col-md-9 mb-3'">
              <label for="last_login_attempt" class="form-label">Last Login Attempt</label>
              <input :value="user.last_login_attempt" id="last_login_attempt" class="form-control" disabled></input>
          </div>
        </div>
        <div v-if="!isEditMode" class="row">
          ...
          <div class="col-md-6 mb-4">
            ...
            <input v-model="matchingPassword" id="password" type="password" class="form-control" required/>
          </div>
        </div>
        <div class="d-flex gap-2">
          <button class="btn btn-secondary w-100" type="button" @click="handleCancel">Cancel</button>
          <button class="btn btn-primary w-100" @click="" type="submit">{{ isEditMode ? 'Update' : 'Create' }}</button>
        </div>
      </form>
    </div>
  </AppLayout>
